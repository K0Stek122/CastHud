...
void Kostek::CastHud::DrawMenu()
{
	...
	ImGui::Checkbox("T Style", (bool*)& m_Profile.TStyle);

	ImGui::SetCursorPos(ImVec2(166.5f, 200.f));
	...
	//ImGui::SameLine();
	//ImGui::Checkbox("Dot", (bool*)& m_Profile.dot);
...
void Kostek::CastHud::DrawCrosshairs()
{
	...
	{
		draw->AddCircle(
			ImVec2(m_width / 2 + m_Profile.xOffset + 1, m_height / 2 + m_Profile.yOffset),
			m_Profile.scale,
...
Profile Kostek::CastHud::LoadConfig()
{
	...
	ret.dot = std::stoi(split[9]);
	ret.TStyle = std::stoi(split[10]);
